...
var zWorkersKvNamespaceGetMultipleKeyValuePairsData = external_exports.object({
  ...
  path: external_exports.never().optional(),
  query: external_exports.object({
    page: external_exports.number().int().gte(1).optional().default(1),
    per_page: external_exports.number().int().gte(1).lte(1e3).optional().default(20)
  }).optional()
...
async function listDONamespaces(c, query) {
  let { page, per_page } = query, doBindingMap = c.env.LOCAL_EXPLORER_BINDING_MAP.do, namespaces = Object.entries(doBindingMap).map(([id, info]) => ({
    id,
  ...
  })), totalCount = namespaces.length, startIndex = (page - 1) * per_page;
  return namespaces = namespaces.slice(startIndex, startIndex + per_page), c.json({
...
async function listDOObjects(c, namespaceId, query) {
  ...
  if (!c.env.LOCAL_EXPLORER_BINDING_MAP.do[namespaceId] || // No loopback service means we can't list DOs
  c.env.MINIFLARE_LOOPBACK === void 0)
...
async function listKVKeys(c, query) {
  let namespace_id = c.req.param("namespace_id"), cursor = query.cursor, limit = query.limit, prefix = query.prefix, kv = getKVBinding(c.env, namespace_id);
  if (!kv)
    return errorResponse(404, 1e4, "Namespace not found");
  let listResult = await kv.list({ cursor, limit, prefix }), resultCursor = "cursor" in listResult ? listResult.cursor ?? "" : "";
  return c.json({
    ...wrapResponse(
